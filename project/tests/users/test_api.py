...
def test_create_user(client, email, status_code):
    ...
    """
    response = client.post('/api/v1/registration/', data={
        'email': email,
        'password': 'secret',
    })
    data = response.json()
...
def test_update_user(client, user):
    ...
    response_2 = client.post('/api/v1/registration/', data={
        'email': 'new_email@mail.ru',
        'password': 'password',
    })
...
def test_update_contact(client, user, contact):
    ...
    """
    response = client.put(f'/api/v1/update_contact/{contact.id}/', data={
        'city': 'test1',
        'street': 'test2',
        'house': 'test3',
    }, headers={
...
def test_get_contacts(client, user, contact):
    ...
    """
    response = client.get('/api/v1/contacts/', headers={
        'Authorization': f'Basic {encode_base64("test_user@mail.ru:password")}',
        'Content-Type': 'application/json'
    })
    data = response.json()
...
def test_delete_contact(client, user, contact):
    ...
    """
    response = client.delete(f'/api/v1/delete_contact/{contact.id}/', headers={
        'Authorization': f'Basic {encode_base64("test_user@mail.ru:password")}',
        'Content-Type': 'application/json'
    })
