...
urlpatterns = [
    ...
    path(
        "registration/", CreateCustomUserViewSet.as_view(), name="registration"
    ),
    # Добавление нового контакта
    path("add_contact/", CreateContactView.as_view(), name="add_contact"),
    # Обновление данных пользователя
    path(
        "update_user/", UpdateCustomUserViewSet.as_view(), name="update_user"
    ),
    # Получение списка контактов
    path("contacts/", GetContactView.as_view(), name="contacts"),
    # Подтверждение email с токеном
    path(
        "confirm_email/<str:token>/<str:email>/",
