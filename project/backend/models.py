    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["external_id", "shop"], name="unique_product_info"
            )
        ]
...
class ProductParameter(models.Model):
    ...
    def __str__(self):
        return (
            f"{self.product_info.model} - {self.parameter.name}: {self.value}"
        )
...
class OrderItem(models.Model):
    order = models.ForeignKey(
        Order, on_delete=models.CASCADE, verbose_name="Заказ", blank=True
    )
    product = models.ForeignKey(
        Product, on_delete=models.CASCADE, verbose_name="Продукт", blank=True
    )
    shop = models.ForeignKey(
        Shop, on_delete=models.CASCADE, verbose_name="Магазин", blank=True
    )
    quantity = models.PositiveIntegerField(verbose_name="Количество")
    ...
    def save(self, *args, **kwargs):
        ...
        self.total_price = (
            self.quantity * self.product.product_info.first().price_rrc
        )
